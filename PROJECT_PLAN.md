...
## Success criteria
- [ ] Next.js 16.2.9 App Router project uses TypeScript, Tailwind v4, Prisma 7.8 WASM client, MariaDB driver adapter, and bcryptjs.
- [ ] Prisma schema represents the full legacy MySQL data model needed for migration, including new `line_user_id` fields.
- [ ] Legacy dropped systems are not reintroduced: no `devices` table and no `teacher_credentials` WebAuthn table.
- [ ] Read-only views can safely inspect migrated data without mutating attendance state.
- [ ] LINE Login identity binding replaces legacy device approval and WebAuthn identity flows.
- [ ] Check-in, teacher admin, and rollout phases are implemented with verification and rollback plans.
## Milestones
...
- [ ] M6: Rollout — plan written in `DEPLOY_PLAN.md`, NOT yet executed. Open items: Plesk SSH/git-deploy status, Plesk subdomain-root-vs-`/lineup/`-subfolder routing verification, production DB provisioning + data sync, LINE Login production channel/callback config, build+deploy, pre-cutover verification pass, cutover communication to teachers/students (device/WebAuthn registrations are gone, everyone re-binds LINE). Also: the working tree has never been git-committed since project init — flagged as a loose end independent of the M6 timeline.
## Key files
